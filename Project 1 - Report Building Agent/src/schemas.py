...
class AnswerResponse(BaseModel):
    ...
    sources: List[str] = Field(
        ...
        description="List of source document IDs used (e.g., ['doc_123', 'page_45']).",
    )
...
class UserIntent(BaseModel):
    ...
    confidence: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Confidence in classification (float between 0 and 1, inclusive)."
    )
    reasoning: str = Field(
        ...,
...
class SessionState(BaseModel):
    ...
    conversation_history: List[BaseMessage] = Field(
        default_factory=list,
        description="The sequence of messages (HumanMessage, AIMessage, etc.) for the current session."
    )
