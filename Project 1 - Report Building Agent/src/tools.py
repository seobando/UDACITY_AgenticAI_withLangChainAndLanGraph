...
def create_calculator_tool(logger: ToolLogger):
    """
    Creates a calculator tool using the @tool decorator.
    This tool safely evaluates mathematical expressions and logs its usage.
    """
    SAFE_EXPRESSION_PATTERN = r"^[\d\s\+\-\*\/\(\)\.]+$"

    @tool
    def calculator(expression: str) -> str:
        """
        Evaluates a mathematical expression and returns the result. 
        Only supports basic operations: +, -, *, /, and parentheses.
        
        Args:
            expression (str): The mathematical expression to evaluate (e.g., "5 * (10 + 2)").
        
        Returns:
            str: The formatted result of the calculation or an error message.
        """
        tool_name = "calculator"
        input_data = {"expression": expression} # Match logger input format

        # 1. Validate the expression for safety
        if not re.match(SAFE_EXPRESSION_PATTERN, expression):
            error_message = f"Error: Expression '{expression}' contains invalid characters or operations. Only basic math (numbers, +, -, *, /, ()) is allowed."
            logger.log_tool_use(tool_name, input_data, error_message)
            return error_message

        try:
            # 2. Evaluate the expression using eval()
            # Note: Safety validation minimizes the risk of using eval() here.
            result_value = str(eval(expression))
            
            # 3. Format the final output string
            formatted_result = f"The result of the calculation is: {result_value}"
            
            # 4. Log the tool usage (SUCCESS)
            logger.log_tool_use(tool_name, input_data, formatted_result)
            
            # 5. Return the formatted result
            return formatted_result

        # 6. Handle calculation errors gracefully (e.g., Division by Zero)
        except ZeroDivisionError:
            error_message = "Error: Cannot divide by zero."
            logger.log_tool_use(tool_name, input_data, error_message)
            return error_message
        except Exception as e:
            error_message = f"Error during calculation: {type(e).__name__} - {e}"
            logger.log_tool_use(tool_name, input_data, error_message)
            return error_message
