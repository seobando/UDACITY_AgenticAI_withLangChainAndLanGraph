...
from langchain_core.messages import (
    BaseMessage,
    HumanMessage, # Internally used
    AIMessage, # Internally used
    SystemMessage, # Internally used
    ToolMessage,
)
import re
import operator
...
def classify_intent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    intent_output: UserIntent = llm_with_structure.invoke(formatted_prompt)

    classified_intent_type = intent_output.intent_type

    next_step = "qa_agent"

    # TODO: Add conditional logic to set next_step based on intent
    if classified_intent_type == "summarization":
        next_step = "summarization_agent"
    elif classified_intent_type == "calculation":
...
def summarization_agent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    """
    # Retrieve necessary components from config
    llm = config.get("configurable").get("llm")
    tools = config.get("configurable").get("tools")

    # 1. Retrieve the summarization prompt template
    prompt_template = get_chat_prompt_template("summarization")

    # 2. Format the prompt with user input and chat history
    messages = prompt_template.invoke(
        {
            "input": state["user_input"],
            "chat_history": state.get("messages", []),
    ...
    # 3. Invoke the agent, enforcing the SummarizationResponse structured output
    result, tools_used = invoke_react_agent(SummarizationResponse, messages, llm, tools)

    # 4. Update and return the state object
    return {
        "messages": result.get("messages", []),
        "actions_taken": ["summarization_agent"],
        # The 'result' is the structured SummarizationResponse
        "current_response": result,
        "tools_used": tools_used,
        "next_step": "update_memory",
    }
...
def calculation_agent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    # 1. Retrieve the calculation prompt template
    ...
    # 3. Invoke the agent, enforcing the CalculationResponse structured output
    ...
    return {
        ...
        # The 'result' is the structured CalculationResponse
        "current_response": result,
        "tools_used": tools_used,
        "next_step": "update_memory",
    }
...
def update_memory(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    return {
        # TODO: Extract summary from response
        "conversation_summary": response.summary,
        # TODO: Update with the current active documents
        "active_documents": response.document_ids,
        # TODO: Update the next step to end
        "next_step": "end",
    }
...
def create_workflow(llm, tools):
    ...
    config = {"configurable": {"llm": llm, "tools": tools}}
